    ...
    constructor(x, y, width, height, hasEnemies, hasPowerup) {
        ...
        if (this.hasEnemies && carl) {
            ...
            for (let i = 0; i < enemyCount; i++) {
                ...
                let crabY = this.y - 30;
                // Only check safe radius if platform is near starting position
                let nearStart = Math.abs(this.y - game.seaLevel) < 800;
                let safeRadius = 250;
                if (!nearStart || dist(crabX, crabY, carl.x, carl.y) > safeRadius) {
...
function generatePlatforms() {
    while (lastPlatformY > game.cameraY - height * 3) {
        ...
        // Don't generate platforms too close to starting position (within 800 pixels)
        if (lastPlatformY > game.seaLevel - 800) {
            continue;
        }
...
function spawnFloatingEnemies() {
    // Grace period: reduced spawn rate in first 3 seconds
    if (game.currentTime < 3 && random() > 0.5) {
        return;
    }
